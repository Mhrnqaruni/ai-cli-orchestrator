...
def check_status():
    ...
    try:
        with open('gemini_status.json', 'r') as f:
            status = json.load(f)
        return status
    except:
        return None

def send_to_gemini(message, max_wait=120):
    """Send a message to Gemini via the bridge and wait for response"""
    print(f"[CLAUDE] Sending to Gemini: {message}")

    # Write command to file
    with open('gemini_command.txt', 'w', encoding='utf-8') as f:
        f.write(message)
        ...
        if status and status.get('status') == 'response_ready':
            ...
            try:
                with open('gemini_response.txt', 'r', encoding='utf-8') as f:
                    response = f.read()
                print(f"\n[CLAUDE] Got response from Gemini:")
                print("=" * 60)
                print(response)
                print("=" * 60)
                return response
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        # Interactive mode
        print("Claude to Gemini Interface")
